...
const { AdequateCacheEntry, AdequateCacheBucket } = require('./internals');
...
class AdequateCacheOptions {
  constructor(source) {
    ...
    /**
     * If using max, we will create buckets to store key data. The trade-off here is, the more buckets
     * you have, the more precise algorithm is, but also, the slower is the access.
     */
...
class AdequateCache {
  constructor(options) {
    ...
    if (this._options.max) {
      ...
      if (!(this._options.bucket_count > 1)) {
        throw new TypeError('When using max size, bucket count must be at least 2');
      }

      for (let i = 0; i < this._options.bucket_count; i++) {
        this._buckets.push(new AdequateCacheBucket());
      }
    }
  }
